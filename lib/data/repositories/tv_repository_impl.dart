...
class TvRepositoryImpl implements TvRepository {
  ...
  Future<Either<Failure, String>> removeWatchlist(TvDetail tv) async {
    try {
      final result =
          await localDataSource.removeTvFromWatchlist(TvTable.fromEntity(tv));
      return Right(result);
  ...
  Future<Either<Failure, String>> saveWatchlist(TvDetail tv) async {
    try {
      final result =
          await localDataSource.insertTvToWatchlist(TvTable.fromEntity(tv));
      return Right(result);
    } on DatabaseException catch (e) {
      return Left(DatabaseFailure(e.message));
    } catch (e) {
  ...
  Future<Either<Failure, List<Episode>>> getEpisodeFromTvSeriesSeason(
    ...
    try {
      final result =
          await remoteDataSource.getEpisodeFromTvSeriesSeason(id, seasonNumber);
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return Left(ServerFailure(''));
    } on SocketException {
